...
static struct {
  ...
  TokenTypeInProjece tok;
    ...
    {"do", DO},     {"endwhile", ENDWHILE}, {"for", FOR},
...
static TokenTypeInProjece checkLogicOp(char* s) {
  if (!strcmp(s, "and")) {
    return AND;
  } else if (!strcmp(s, "or")) {
    return OR;
 ...
 */
TokenTypeInProjece getToken(void) { /* index for storing into tokenString */
  ...
  while (state != DONE) {
    ...
    if (state == DONE) {
      ...
      if (currentToken == ID) {
        currentToken = checkLogicOp(tokenString);
        if (currentToken == ID)
